...
func (baseMenuService *BaseMenuService) DeleteBaseMenu(id int) (err error) {
	err = global.GVA_DB.Preload("MenuBtn").Preload("Parameters").Where("parent_id = ?", id).First(&system2.SysBaseMenu{}).Error
	if err != nil {
		var menu system2.SysBaseMenu
		db := global.GVA_DB.Preload("SysAuthoritys").Where("id = ?", id).First(&menu).Delete(&menu)
		err = global.GVA_DB.Delete(&system2.SysBaseMenuParameter{}, "sys_base_menu_id = ?", id).Error
		err = global.GVA_DB.Delete(&system2.SysBaseMenuBtn{}, "sys_base_menu_id = ?", id).Error
		err = global.GVA_DB.Delete(&system2.SysAuthorityBtn{}, "sys_menu_id = ?", id).Error
		if err != nil {
			return err
		}
...
func (baseMenuService *BaseMenuService) UpdateBaseMenu(menu system2.SysBaseMenu) (err error) {
...
func (baseMenuService *BaseMenuService) GetBaseMenuById(id int) (menu system2.SysBaseMenu, err error) {
