...
func (autoCodeService *AutoCodeService) PreviewTemp(autoCode system2.AutoCodeStruct) (map[string]string, error) {
...
func makeDictTypes(autoCode *system2.AutoCodeStruct) {
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system2.AutoCodeStruct, ids ...uint) (err error) {
	makeDictTypes(&autoCode)
	for i := range autoCode.Fields {
		if autoCode.Fields[i].FieldType == "time.Time" {
			autoCode.HasTimer = true
...
func (autoCodeService *AutoCodeService) AutoCreateApi(a *system2.AutoCodeStruct) (ids []uint, err error) {
	apiList := []system2.SysApi{
		{
...
func (autoCodeService *AutoCodeService) getNeedList(autoCode *system2.AutoCodeStruct) (dataList []tplData, fileList []string, needMkdir []string, err error) {
	// 去除所有空格
...
func (autoCodeService *AutoCodeService) CreateAutoCode(s *system2.SysAutoCode) error {
	if s.PackageName == "autocode" || s.PackageName == "system" || s.PackageName == "example" || s.PackageName == "" {
		return errors.New("不能使用已保留的package name")
	}
	if !errors.Is(global.GVA_DB.Where("package_name = ?", s.PackageName).First(&system2.SysAutoCode{}).Error, gorm.ErrRecordNotFound) {
...
func (autoCodeService *AutoCodeService) GetPackage() (pkgList []system2.SysAutoCode, err error) {
	err = global.GVA_DB.Find(&pkgList).Error
	return pkgList, err
}

func (autoCodeService *AutoCodeService) DelPackage(a system2.SysAutoCode) error {
...
// CreatePlug 自动创建插件模板
func (autoCodeService *AutoCodeService) CreatePlug(plug system2.AutoPlugReq) error {
	// 检查列表参数是否有效
	plug.CheckList()
