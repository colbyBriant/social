...
import (
	...
	"github.com/flipped-aurora/gin-vue-admin/server/model/entity/bbs"
	"github.com/flipped-aurora/gin-vue-admin/server/model/entity/bbs/request"
)
...
func (cbbs *XkBbsService) UpdateXkBbs(xkBbs *bbs.XkBbs) (err error) {
	err = global.GVA_DB.Save(xkBbs).Error
	//err = global.GVA_DB.Model(xkBbs).Updates(xkBbs).Error
	return err
}
...
// @return: err error
func (cbbs *XkBbsService) DeleteXkBbs(xkBbs *bbs.XkBbs) (err error) {
...
// @return: err error
func (cbbs *XkBbsService) DeleteXkBbsById(id uint) (err error) {
	var xkBbs bbs.XkBbs
	err = global.GVA_DB.Where("id = ?", id).Delete(&xkBbs).Error
	return err
}
...
func (cbbs *XkBbsService) GetXkBbs(id uint) (xkBbs *bbs.XkBbs, err error) {
...
func (cbbs *XkBbsService) LoadXkBbsPage(info request.BbsPageInfo) (list interface{}, total int64, err error) {
	// 获取分页的参数信息
	limit := info.PageSize
